...
<style>
    ...
    .btn-close {
    background: rgba(0, 0, 0, 0.5); /* Background to make it visible */
    border: none; /* No border */
    color: rgb(255, 255, 255); /* Cross color */
...
@section('content')
    ...
    @include('backend.pages.gallery_photo.modal')
@endsection
@section('script')
    ...
    var table = jQuery('#dataTable').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: "{{ route('gallery.photo.list') }}", // Ensure this route matches your controller
            type: 'GET',
        },
        aLengthMenu: [
    ...
    if (go_next_step == true) {
        ...
        $.ajax({
            ...
            url: "{{ route('gallery.photo.store') }}",  // Ensure this route matches
            type: "POST",
            data: formData,
            processData: false,
            contentType: false,
        ...
        $(document).on('click', '#editBtn', function(e) {
            ...
            if (go_next_step == true) {
                ...
                $.ajax({
                    ...
                    url: "{{ route('gallery.photo.update') }}",
                    type: "POST",
                    data: formData,
                    processData: false,
                    contentType: false,
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        url: "{{ route('gallery.photo.delete') }}",
                        data: {
                            id: id
                        },
                        type: "post",
